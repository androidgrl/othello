...
const instantiateGameObjects = require('./instantiate');
...
Board.prototype.renderBoard = function(canvasElement) {
    ...
    if (this.game.over === false) {
        ...
        if (blackScore.length > whiteScore.length) {
            ...
            $('#turn').append(`<h1> ${this.game.players[0].name} wins with a score of ${whiteScore.length}!! </h1>`);
        }
        $('#turn').prepend(`<button id="play-again">Play Again</button>`);
