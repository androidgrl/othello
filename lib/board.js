...
Board.prototype.respondToClick = function(x, y) {
    //after the click, check to see of there are valid moves for the current player
    //if there are not any then add to the counter and add to the turn counter update
    //the current player
    //if the other player does not have any valid moves, then game over
    //if the other player does have valid moves, then set the counter to zero
    ...
    if (result) {
        ...
        if (validSquares.length > 0) {
            this.game.counter = 0;
        } else if (validSquares.length === 0 && this.game.counter === 1) {
            alert("Game Over");
        } else {
            this.game.counter++;
            this.game.turn++;
