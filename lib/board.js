...
Board.prototype.findValidSquares = function() {
    var playerColor = this.game.currentPlayer.color;

    var emptySquares = this.squares.filter(function(square){
        return square.piece === null;
        //if the square is empty && the square's neighbors have opposite colored pieces
    });

    var emptyHasOppositeColorTouching = emptySquares.filter(function(square){
        //todo
        //square.neighbors
        return square;
    });
    //var validSquares = occupiedSquares.map(function(){
    //});
